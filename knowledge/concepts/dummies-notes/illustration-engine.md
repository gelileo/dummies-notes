...
**Progressive reveal (optional):** a frame may carry `beats: [{caption, narration}, …]` in `figure.json` and use `data-reveal="k"` on `<g>` elements in its SVG to reveal content one step at a time. `data-reveal` is a 1-based integer; `data-anim="rise|draw|fade"` (default `rise`) controls the entrance style — `draw` for `<line>` and arrows, `rise` for shapes and boxes, `fade` for cross-fades. Groups without `data-reveal` form the always-visible backdrop. `len(beats)` must equal `max(data-reveal)` and indices must be gap-free `1..N`; the validator enforces both. A frame with neither `data-reveal` nor `beats` is a single beat — fully backward-compatible. Canonical example: `examples/tcp-handshake-reveal` (6 beats, one frame, three draw arrows + two rise boxes + one rise badge).
...
**Acronym allowlist**: `check_caps` exempts short technical acronyms from the ALL-CAPS error. A module-level `CAPS_ACRONYMS` set (SYN, ACK, FIN, RST, TCP, UDP, IP, HTTP, HTTPS, DNS, TLS, SSL, RSA, AES, API, URL, URI, HTML, CSS, JSON, XML, SQL, CPU, GPU, RAM, ID, OK, IO, OS) lists allowed tokens. When every all-caps alphabetic token of length ≥ 2 in a text element is present in this set, the element is exempt; if any such token is absent, the ALL-CAPS error fires as before. Single-letter tokens (axis labels, variables) are also exempt by the length ≥ 2 guard. Genuine shouting ("BAD", "HELLO WORLD") and unknown-acronym text continue to flag.

**Reveal-consistency lint** (`_lint_reveal`): `validate_figure` also checks that `data-reveal` groups in a frame's SVG are consistent with the `beats` array in `figure.json`. Rules: (1) if a frame has `data-reveal` groups but no `beats`, that is an ERROR; (2) if a frame has `beats` but no `data-reveal` groups, that is an ERROR; (3) `data-reveal` indices must form a gap-free `1..N` sequence (missing indices are an ERROR); (4) the count of `beats` must equal the max `data-reveal` value (mismatch is an ERROR); (5) `data-reveal` values must be integers (a non-integer like `"abc"` is an ERROR). Frames with neither `data-reveal` nor `beats` are silently skipped — existing figures without progressive reveal validate clean. **Non-`<g>` authoring guard (WARN):** if `data-reveal` is found on an element whose local name is not `g` (e.g. `<rect data-reveal="1">`), a WARN is emitted: "only `<g>` groups reveal in the MP4; wrap it in a `<g>`". This is intentionally WARN-not-ERROR: the index counts/gap checks still run; the validator does not fail validation for this alone.
