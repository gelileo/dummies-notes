...
**Phase 7 Task 3 (cumulative-state SVG for MP4 pop).** `_reveal_svg(inner, reveal_to)` hides SVG groups whose `data-reveal` index exceeds `reveal_to` by injecting `style="visibility:hidden"` inline on the opening `<g>` tag — rasterizer-agnostic, no reliance on CSS attribute selectors. The style is inserted right after `<g ` (before `data-reveal`) so positional window assertions in tests are satisfied. `reveal_to=None` passes through unchanged (backward-compat for beat-less frames). `stage_svg` now calls `_reveal_svg` before `_nest_figure` for frame slides, keying off `slide.get("reveal_to")`. Non-frame slides are unaffected. Regex `_REVEAL_G_RE` only touches `<g>` opening tags with `data-reveal`. Existing `style=` attributes are prefixed rather than replaced. Three new tests in `TestRevealSvg`. 31 tests, 1 skip, all passing.

**Phase 7 Task 5 (HTML player reveal engine — per-frame container grouping).** The HTML player was rewritten to support progressive reveal of beat-slides that share one figure image. Key changes:

- **Container model** (`_containers(slides)`): consecutive `"frame"` slides sharing the same `image` path collapse into a single DOM `.slide` container — the figure SVG is inlined **once** per unique image. Every non-frame slide gets its own container. Returns `(containers, idx)` where `idx[i]` is the container index for slide `i`.
- **`_container_html(c)`**: renders each container as a `.slide` div — frame containers inline the raw SVG via `_read_inner_svg`; card containers render a `.card` div with escaped text.
- **`build_player`**: now calls `_containers` instead of iterating slides directly. The injected manifest's `slides` array gains two new fields per slide: `reveal_to` (int|None, progressive level) and `container` (int, DOM index). The HTML template is substituted with containers-based `SLIDES_HTML`.
- **`_slide_html` removed**: the per-slide HTML helper is gone; `_container_html` replaces it.
- **Player template** (`.claude/skills/concept-illustrator/assets/video.template.html`): completely replaced with a reveal engine. The JS now queries `#stage > .slide` as DOM containers, not one-per-slide. `show(n)` toggles `.show` on `conts[s.container]` and calls `reveal(cont, s.reveal_to)` to animate entrance of `[data-reveal]` groups. `reveal()` adds/removes `.on` (visible, animated) and `.dim` (dimmed past beats) classes. `drawLines(g)` provides a cubic-ease SVG line-draw animation for `data-anim="draw"` groups. A "dim past" checkbox lets the viewer control whether past beats fade. The caption is now a fixed `#cap` element outside the slide containers, updated by `show()`.
- **Light manifest fields**: `kind`, `concept_slug`, `caption`, `narration`, `duration_s`, `transition`, `reveal_to`, `container` — no SVG text in the JSON.
